...
import { getBuyer } from "@/demo/data";
import {
  validateGrade,
  applyDecay,
  daysSinceContact,
...
export default function BuyerDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const buyer = getBuyer(id);
  const [grade, setGrade] = useState<BuyerGrade>(buyer?.grade ?? "C");
  ...
  if (!buyer) {
    return (
      <div className="p-8 text-fg-muted">
        Buyer not found. <Link href="/buyers" className="text-accent">Back to buyers</Link>
      </div>
    );
  }
  ...
  // If this buyer decayed, restoring to a higher grade requires a fresh note.
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-5 p-5 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
          {/* Grade control with POF validation */}
          <Card>
            <CardHeader>
                ...
                <div key={i} className="flex items-center gap-2 text-[13px]">
                  <span
                    className={`h-3.5 w-3.5 rounded border ${
                      buyer.disqualifiers[i] ? "border-danger bg-danger" : "border-border"
                    }`}
                  />
        ...
        {/* Right rail: POF, decay countdown, certainty */}
          ...
          <Card>
            ...
            <CardBody className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <Badge tone={buyer.pofStatus === "received" ? "ok" : buyer.pofStatus === "pending" ? "warn" : "danger"}>
                  {buyer.pofStatus}
                </Badge>
              </div>
              <div className="flex items-center justify-between">
            ...
            <CardBody className="text-sm">
              ...
              {countdown != null && (
                <div className={`mt-2 text-xs ${countdown <= 0 ? "text-warn" : "text-fg-muted"}`}>
                  {countdown <= 0
                    ? "Past the 90-day decay threshold."
                    : `${countdown} days until auto-decay.`}
                </div>
              )}
            </CardBody>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Badge tone={certainty.score >= 70 ? "ok" : certainty.score >= 40 ? "warn" : "danger"}>
                  {certainty.score}/100
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardBody className="space-y-1">
              ...
              <p className="pt-1 text-[10px] text-fg-faint">
                Buyers rank by certainty, not headline price.
              </p>
            </CardBody>
          </Card>
        </div>
