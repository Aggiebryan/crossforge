...
export function PipelineClient({ leads, live }: { leads: LeadView[]; live: boolean }) {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${leads.length} leads · ${live ? "live data" : "demo data"} · 18 fixed stages`}
        actions={
          <div className="flex items-center gap-2">
            <Link href="/pipeline/new?mode=quick">
            ...
            <div className="flex rounded-md border border-border p-0.5">
              <button
                ...
                className={cn("rounded px-3 py-1 text-xs font-medium", view === "kanban" ? "bg-accent text-accent-fg" : "text-fg-muted")}
              >
                Kanban
              </button>
              <button
                onClick={() => setView("table")}
                className={cn("rounded px-3 py-1 text-xs font-medium", view === "table" ? "bg-accent text-accent-fg" : "text-fg-muted")}
              >
                Table
              </button>
            </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center p-16 text-center">
      ...
      <div className="mt-3 flex gap-2">
        <Link href="/pipeline/new">
          <Button className="gap-1.5">
            <Plus className="h-3.5 w-3.5" /> Create your first lead
          </Button>
        </Link>
        <Link href="/pipeline/new?mode=quick">
          <Button variant="outline">Quick capture</Button>
        </Link>
      </div>
    </div>
  );
...
function Kanban({ leads }: { leads: LeadView[] }) {
  return (
          ...
          return (
            <div key={stage} className="w-64 shrink-0">
              <div className="mb-2 flex items-center justify-between px-1">
                <span className="text-xs font-semibold text-fg-muted">{LEAD_STAGE_LABELS[stage]}</span>
                <span className="text-[11px] text-fg-faint">{col.length}</span>
              </div>
              <div className="space-y-2">
                {col.map((l) => (
                  <Link key={l.id} href={`/pipeline/${l.id}`} className="block rounded-md border border-border bg-bg-elev p-3 hover:border-border-strong">
                    <div className="text-sm font-medium text-fg leading-tight">{l.street || l.property_address}</div>
                ...
                {col.length === 0 && (
                  <div className="rounded-md border border-dashed border-border py-6 text-center text-[11px] text-fg-faint">empty</div>
                )}
              </div>
            </div>
          );
...
function Table({ leads }: { leads: LeadView[] }) {
  return (
    <div className="p-4">
